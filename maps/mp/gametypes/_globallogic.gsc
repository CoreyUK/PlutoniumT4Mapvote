...
endGame( winner, endReasonText )
{
	...
	mapVoteIntermission = getDvarInt( "mv_enable" );
	if ( mapVoteIntermission )
		println( "[T4 Map Vote] Stock scoreboard intermission bypass active" );
	...
	for ( index = 0; index < players.size; index++ )
	{
		...
		if ( mapVoteIntermission )
		{
			// spawnIntermission() sets the client session state to "intermission",
			// which forces T4's scoreboard to remain drawn until the player presses
			// Tab. Keep map-vote clients as spectators so the HUD vote remains on top.
	...
	// popup for game summary (the map vote replaces it at a voted final game)
	if ( !mapVoteIntermission )
	{
		players = level.players;
		for ( index = 0; index < players.size; index++ )
